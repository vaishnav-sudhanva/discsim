...
def run_simulation_task(params: dict, task_id: str):
    ...
    try:
        ...
        l0_summary_path, l1_summary_path = analytics_ranker.process_ranking_analytics(
        ...
        tasks_db[task_id] = {
            "status": "Complete",
            "l0_summary_path": l0_summary_path,
            "l1_summary_path": l1_summary_path,
...
def check_nested_sim(task_id: str):
    ...
    When complete, it decodes both Decoupled Parquet Summary Files safely for UI rendering.
    """
    task = tasks_db.get(task_id, {"status": "Failed", "error": "Task identification token not found."})
    
    if task["status"] == "Complete":
        # Pull completed binary storage matrices up via pandas high-speed parquet readers
        df_l0_summary = pd.read_parquet(task["l0_summary_path"])
        df_l1_summary = pd.read_parquet(task["l1_summary_path"])
        
        # Clean both data frames to shield the web protocol from NaN/Infinity serialization issues
        clean_l0_records = clean_json_records(df_l0_summary)
        clean_l1_records = clean_json_records(df_l1_summary)
        
        # Return both operational summary dictionaries down to the waiting user dashboard state
        return {
            "status": "Complete",
            "clinic_summary_data": clean_l0_records,       # Pre-sorted, lightweight clinic metrics
            "supervisor_summary_data": clean_l1_records,   # Pre-sorted, lightweight supervisor metrics
