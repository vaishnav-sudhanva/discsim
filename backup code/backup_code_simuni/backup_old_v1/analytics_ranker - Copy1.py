...
# ==============================================================================
def calculate_overlap_percentage(group_df, target_pct, id_cols, truth_sort_col, sample_sort_col):
    """
    Calculates how successfully a Field Worker's physical sample caught the True Worst fraudsters.
    FIXED: Now correctly enforces the "Penalty of Ignorance" by tracking unvisited clinics.
    """
    ...
    # --------------------------------------------------------------------------
    # Count the total units (e.g., all 25 clinics) BEFORE dropping any unvisited rows.
    # This guarantees the threshold denominator is never artificially shrunk.
    total_units_global = group_df[uid_col].nunique()
    
    # Calculate how many units make up the "Target Percentile" (e.g., 30% of 25 = 7 clinics)
    k_threshold = max(1, int(np.round(total_units_global * target_pct)))
    
    # Find the True Worst units by sorting the designated Truth column in descending order.
    # Secondary sort by ID prevents arbitrary ties from ruining reproducibility.
    true_worst_df = group_df.sort_values(by=[truth_sort_col, uid_col], ascending=[False, True]).head(k_threshold)
    true_worst_set = set(true_worst_df[uid_col]) # Store as a Set for ultra-fast overlap math
    ...
    # If they didn't visit it, the sample column is NaN.
    clean_df = group_df.dropna(subset=[sample_sort_col]).copy()
    
    # If their budget/strategy resulted in 0 valid visits, their catch rate is instantly 0%.
    if clean_df.empty:
        return 0.0
        
    # Find what the Field Worker thinks are the worst units, based ONLY on the ones they actually measured.
    caught_worst_df = clean_df.sort_values(by=[sample_sort_col, uid_col], ascending=[False, True]).head(k_threshold)
...
def process_ranking_analytics(l0_parquet_path, l1_parquet_path, output_dir, task_id, target_percentiles=[0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90]):
    ...
    # The columns that perfectly define a unique "What If" simulation slice
    slice_cols = ['Indicator', 'Sim_ID', 'L1_Budget_Pct', 'L1_Label', 'L2_Budget_Pct', 'L2_Label']
    ...
    for keys, group in tqdm(df_l0.groupby(slice_cols), desc="L0 (Clinic) Analysis", leave=False):
        ind, sim, l1_b, l1_lbl, l2_b, l2_lbl = keys
        
        for pct in target_percentiles:
            # FIX: V2 Must be evaluated INTRA-DISTRICT (Supervisor by Supervisor).
            # A Supervisor only ranks their OWN clinics, not clinics on the other side of the country.
            v2_acc_list = []
            
            for l1_id, l1_group in group.groupby('L1_id'):
                acc = calculate_overlap_percentage(
                    group_df=l1_group,         # Pass only the 25 clinics belonging to THIS Supervisor
                    target_pct=pct,
                    id_cols='L0_id',           # Rank the L0s
                    truth_sort_col='Pop_L0_Real_MAE', # The Absolute God-Mode Baseline Truth
                    sample_sort_col='Samp_L1_L0_MAE'  # The physical difference L1 found during their visit
                )
                v2_acc_list.append(acc)
                
            # Average the accuracy across all 100 Supervisors to get the Global V2 Score for this simulation run
    ...
    for keys, group in tqdm(df_l1.groupby(slice_cols), desc="L1 (Supervisor) Analysis", leave=False):
        ind, sim, l1_b, l1_lbl, l2_b, l2_lbl = keys
        
        for pct in target_percentiles:
            
            # ------------------------------------------------------------------
            # V1: THE GOD-MODE BENCHMARK
            # Assumption: Can the Auditor (L2) find the supervisors who are TRULY the worst globally?
            # ------------------------------------------------------------------
            v1_acc = calculate_overlap_percentage(
                group_df=group,            # Pass all 100 Supervisors globally
                target_pct=pct,
                id_cols='L1_id',
                truth_sort_col='Pop_Dist_L1_Real_MAE', # The True Fraud across ALL 375 kids in the district
                sample_sort_col='Samp_Dist_L2_L1_MAE'  # The Discrepancy L2 found in their tiny nested audit
            )
            
            # ------------------------------------------------------------------
            # V3: THE NESTED REALITY BENCHMARK
            # Assumption: Can the Auditor (L2) find the supervisors who lied the most 
            # *specifically* regarding the kids that the Supervisor actually reported on?
            # ------------------------------------------------------------------
            v3_acc = calculate_overlap_percentage(
                group_df=group,
                target_pct=pct,
                id_cols='L1_id',
                truth_sort_col='Samp_Dist_L1_Real_MAE', # FIX: Graded strictly against the L1 Nested Population
                sample_sort_col='Samp_Dist_L2_L1_MAE'   # The Discrepancy L2 found in their tiny nested audit
            )
