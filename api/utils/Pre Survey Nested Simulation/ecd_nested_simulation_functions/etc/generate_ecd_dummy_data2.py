...
def generate_real_measurements(
    ...
    figsize = [10, 8],
    verbose = False,
    rho= 0.0 # VS Code addition - Generate WHZ using Bivariate Normal Distribution - add rho parameter for correlation between haz and waz growth rates
...
# ==============================================================================
    # INITIAL HAZ AND WAZ GENERATION (Updated for Bivariate Biological Correlation)
    # ==============================================================================
    
    # 1. Resolve Means and Standard Deviations for Height
    if (haz_mean_0 is None or haz_std_0 is None) and percent_stunting is None:
        raise ValueError("At least one of haz_mean_0 or percent_stunting must be provided.")

    if haz_mean_0 is not None and haz_std_0 is not None:
        final_haz_mean = haz_mean_0
        final_haz_std = haz_std_0
    elif percent_stunting is not None:
        final_haz_mean = -2 - norm.ppf(percent_stunting/100)
        final_haz_std = 1.0

    # 2. Resolve Means and Standard Deviations for Weight
    if (waz_mean_0 is None or waz_std_0 is None) and percent_underweight is None:
        raise ValueError("At least one of waz_mean_0 or percent_underweight must be provided.")

    if waz_mean_0 is not None and waz_std_0 is not None:
        final_waz_mean = waz_mean_0
        final_waz_std = waz_std_0
    elif percent_underweight is not None:
        final_waz_mean = -2 - norm.ppf(percent_underweight/100)
        final_waz_std = 1.0

    # 3. Generate the Anthropometry 
    # If rho == 0.0, this mathematically acts exactly like the old independent np.random.normal() method.
    covariance = rho * final_haz_std * final_waz_std
    cov_matrix = [
        [final_haz_std**2, covariance],
        [covariance,       final_waz_std**2]
    ]
    
    # Generate Bivariate Normal Distribution simultaneously
    joint_scores = np.random.multivariate_normal([final_haz_mean, final_waz_mean], cov_matrix, num_children)
    haz_0 = joint_scores[:, 0]
    waz_0 = joint_scores[:, 1]

    # Recalculate percent stunting/underweight based on the generated population
    percent_stunting = (haz_0 < -2).mean() * 100
    percent_underweight = (waz_0 < -2).mean() * 100
...
def generate_L0_distorted_measurements(
        ...
        **kwargs  # <--- JUST ADD THIS LINE (WITH THE COMMA ABOVE IT)
        ):
...
def generate_nested_distortion_parameters(
    ...
    for L1_id in tqdm(range(n_L1s), desc="Assigning Personalities"): # <--- WRAP WITH TQDM
    # for L1_id in range(n_L1s):
        # Generate unit means
        unit_means = {
        ...
        #     L0_params = {
        for L0_id in range(n_L0s_per_L1): # <--- CHANGE '_' TO 'L0_id'
            L0_params = {
                'L1_id': f'L1_{L1_id}', # <--- ADD THIS LINE
                'L0_id': f'L0_{L0_id}', # <--- ADD THIS LINE
                'percent_under_reporting_stunting': (
