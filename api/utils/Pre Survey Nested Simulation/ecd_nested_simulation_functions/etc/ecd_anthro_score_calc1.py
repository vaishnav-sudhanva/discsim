...
def height_from_haz(haz, age, sex, haz_params):
    ...
    # 1. Get the biological parameters
    m, s, l = get_msl(age, sex, haz_params)
    
    # 2. Calculate the raw height/length
    height = m * (np.power((haz * l * s) + 1, 1/l))
    
    # 3. BIOLOGICAL POSITION ADJUSTMENT (The -0.65 HAZ Fix)
    # At 730 days (2 years), measurement switches from Lying to Standing.
    # We subtract 0.7cm for children >= 730 days to stay on the correct growth curve.
    mask_standing = (age >= 730)
    height[mask_standing] -= 0.7
    
    return pd.Series(height, index=haz.index)
...
"""
Vectorization Upgrade: Replaced slow, one-by-one dictionary loops with np.interp to process thousands of children simultaneously.
Precision Improvement: Switched from exact-match lookups to linear interpolation, allowing for accurate parameters at fractional ages or heights.
Index Safety: Implemented a check for hasattr(age, 'index') to ensure the function can handle both raw Numpy arrays and Pandas Series without crashing.
Dynamic Column Support: Added age_label and sex_label parameters to allow the engine to query different WHO tables (Age, Length, or Height).
Structural Impact: Safe; the function signature remains backwards-compatible while significantly increasing execution speed.
"""
